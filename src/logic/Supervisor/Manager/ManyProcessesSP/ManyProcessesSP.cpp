...
ManyProcessesSP::ManyProcessesSP(pid_t starter_pid) : Supervisor(starter_pid) {
    curr_syscalls_n = 0;
}
...
void ManyProcessesSP::handle_syscall(seccomp_notif *req, seccomp_notif_resp *resp, int notifyFd)
{
    resp->error = 0;
    resp->val = 0;
...
int ManyProcessesSP::addRuleUnsync(pid_t pid, Rule rule, std::vector<int> syscalls)
{
...
void ManyProcessesSP::deleteRuleUnsync(int rule_id)
{
...
int ManyProcessesSP::updateRule(pid_t pid, int rule_id, Rule rule)
{
...
void ManyProcessesSP::ruleInit(pid_t pid) {
    sem_wait(this->semaphore);
